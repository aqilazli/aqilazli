# Aqil Azli | Test Engineer 🤖🧠 ---> AI Robotics Engineer

Industrial Automation • Robotics • PLC Systems • AI Integration • Mechatronics • 3D Engineering Design  

---

## 👨‍💻 Professional Profile

Mechatronics and AI engineer with hands-on experience in robotics, industrial automation, and intelligent control systems. Skilled in integrating hardware and software to develop smart, scalable, and efficient automation solutions aligned with **Industry 4.0 standards**.  

Focused on designing, programming, and deploying AI-driven robotics systems for industrial applications, with strong emphasis on reliability, precision, and performance.  

---

## ⚙️ Key Skills

### Industrial Automation & PLC
- PLC programming and ladder logic  
- Sensor & actuator integration  
- Automation workflow design & optimization  
- Industry 4.0 system architecture  

### Robotics Engineering
- Robotic arm control and kinematics  
- Raspberry Pi-based automation controllers  
- Motor drivers, sensors, Arduino and embedded electronics  
- ROS2 and Python-based robotics frameworks  

### Mechatronics Systems
- Hardware-software integration  
- Embedded system design and troubleshooting  
- Sensor feedback & real-time monitoring  
- Prototyping and mechanical-electrical coordination  

### AI & Intelligent Systems
- Python-based AI automation scripts  
- Local LLM deployment for robotics intelligence  
- AI-assisted decision making and control systems  
- Machine learning integration for real-time data  

### 3D Design & Visualization
- SketchUp modeling for mechanical concepts  
- Enscape / Lumion rendering for visualization  
- Prototype simulation and presentation  

---

## 🛠 Technical Stack

**Programming:** Python | Bash | XML  
**Hardware & Platforms:** Raspberry Pi | Embedded Systems | PLC Controllers | Industrial Sensors  
**Design Tools:** SketchUp | Enscape | Solidworks | Ironcad 
**Concepts:** Robotics Kinematics | AI Integration | Mechatronics | Automation Architecture | Industry 4.0  

---

## 🚀 Current Projects & Focus

- AI-powered robotic arm control systems  
- Hybrid PLC + AI industrial automation solutions  
- Real-time sensor monitoring dashboards  
- Smart factory automation prototypes  

---

## 🎯 Career Objective

To contribute to *Industrial automation and AI robotics projects** by delivering AI-driven, intelligent and reliable systems, bridging the gap between advanced robotics, AI and Industry 4.0 standards.  

---

## 📫 Contact

Open to collaboration, internships, or full-time roles in:  
- Robotics & Automation  
- AI-integrated Industrial Systems  
- Mechatronics & Embedded Systems  

LinkedIn: [https://linkedin.com/in/muhammad-aqil-azli]  
Email: [aqil.azli@outlook.com]  

